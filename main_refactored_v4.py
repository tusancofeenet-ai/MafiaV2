...
```
This class currently keeps the legacy MafiaApplication as its base
for compatibility with the existing runtime. The legacy callback
handlers are removed before the v4 feature modules register their
own handlers.
"""
...
def __init__(self, token: str):
    ...
    # This must happen before ScenarioManagementV4 / FeatureParityV4
    # registration so their handlers are not accidentally removed.
...
def _disable_legacy_lobby_handlers(self) -> None:
    """Remove legacy callback handlers while preserving v4 registration.

    aiogram 2.x stores callback handlers in ``handlers``.  The current
    migration architecture requires the legacy callback table to be
    cleared before v4 modules register their handlers.

    Only the explicitly retained challenge-toggle handlers are restored
    here. All other production lobby handlers must belong to the v4
    registration layer.
    """
...
async def _render_lobby(self, group_id: int):
    ...
    message_id = getattr(self.ui, "lobby_message_id", None)

    # No existing lobby message: create one.
    if not message_id:
        try:
    ...
    # Existing lobby message: update it.
    try:
    ...
    # The stored message may have been deleted or become invalid.
    # Create a replacement and update the stored message ID.
    try:
        message = await self.bot.send_message(
            group_id,
            text,
